 ...
 * 2. FTS5 全文检索 (message_node_fts)
 * 3. 逐字素材库 (verbatim_artifact)
 * 4. 会话级归档回填记录 (lastArchiveRecallIds)
 *
 * 目的：
        ...
        try {
            ...
            while (cursorConv.moveToNext()) {
                ...
                while (cursorNodes.moveToNext()) {
                    ...
                    // 6.3 解析 JSON，提取 currentMessage 的 role 和 text
                    try {
                        ...
                        if (messagesArray.length() > 0) {
                            ...
                            if (rawText.isNotEmpty()) {
                                ...
                                // 6.5 写入 message_node_text（触发器自动同步 FTS）
                                db.execSQL(
                ...
                // 6.6 扫描 role=0 (USER) 的消息，生成 verbatim_artifact
                ...
                while (cursorUserNodes.moveToNext()) {
                    ...
                    if (artifactType != null) {
                        ...
                        // 6.10 写入 verbatim_artifact（每条 artifact 只覆盖单节点，start==end）
                        db.execSQL(
